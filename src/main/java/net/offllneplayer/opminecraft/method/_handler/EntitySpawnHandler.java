...
/*--------------------------------------------------------------------------------------------*/
    ...
    private static ItemStack getBruteAxe() {
        ...
        return switch (pick) {
            ...
            case 2 -> new ItemStack(RegistryIBBI.CRYING_AXE.get());
            case 3 -> new ItemStack(RegistryIBBI.CRYING_SWHOPAXEL.get());
            case 4 -> new ItemStack(RegistryIBBI.CRYING_PAXEL.get());
    ...
    private static void applyWeaponEnchantments(Level level, ItemStack weapon) {
        ...
        // Override for paxels bc they are diggerItem
        if (witem == RegistryIBBI.CRYING_PAXEL.get() || witem == RegistryIBBI.CRYING_SWHOPAXEL.get()) {
            isSwordLike = true;
            isAxeLike = true;
        }
        ...
        if (isSwordLike) {
            ...
            if (RANDOM.nextInt(10) == 4) applyRandomLevelEnchant(level, weapon, Enchantments.KNOCKBACK, 2);

            if (RANDOM.nextInt(20) == 4) {
                applyRandomLevelEnchant(level, weapon, Enchantments.SWEEPING_EDGE, 3);
            } else
                if (RANDOM.nextInt(20) == 4) applyRandomLevelEnchant(level, weapon, RegistryEnchantments.TEMPEST, 1);
        }

        if (isAxeLike) {
            if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, weapon, Enchantments.EFFICIENCY, 5);
            ...
            if (fortune) {
                ...
                if (silk) applyRandomLevelEnchant(level, weapon, Enchantments.SILK_TOUCH, 1);
        }else{
            if (witem == Items.TRIDENT) {
                if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, weapon, Enchantments.IMPALING, 5);
                if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, weapon, Enchantments.UNBREAKING, 3);
                ...
                } else if (RANDOM.nextInt(20) == 0) applyRandomLevelEnchant(level, weapon, Enchantments.RIPTIDE, 3);

                if (RANDOM.nextInt(10) == 0) {
                    applyRandomLevelEnchant(level, weapon, Enchantments.CHANNELING, 1);
                } else
                    if (RANDOM.nextInt(10) == 0) applyRandomLevelEnchant(level, weapon, RegistryEnchantments.TEMPEST, 3);
                }
            }
        }
    ...
    private static ItemStack getRandomZombieItem(Mob mob) {
        for (int attempt = 0; attempt < 4; attempt++) {
            Item miscItem = switch (RANDOM.nextInt(11)) {
                case 0 -> Items.ICE; case 1 -> Items.PACKED_ICE; case 2 -> Items.BLUE_ICE;
                case 3 -> Items.BLAZE_ROD; case 4 -> Items.BLAZE_POWDER; case 5 -> Items.LAVA_BUCKET;
                case 6 -> Items.SPIDER_EYE; case 7 -> Items.POISONOUS_POTATO;
                case 8 -> Items.FIRE_CHARGE; case 9 -> Items.WIND_CHARGE;
                case 10 -> RegistryIBBI.AKU_AKU_MASK.get();
                default -> Items.POISONOUS_POTATO;
            };

            if (mob instanceof Drowned &&
                    miscItem == Items.BLAZE_POWDER || miscItem == Items.BLAZE_ROD || miscItem == Items.FIRE_CHARGE || miscItem == Items.LAVA_BUCKET) continue;
...
private static final String[] MATERIALS = {"leather", "chainmail", "iron", "golden", "diamond", "netherite"};
    ...
    private static void applyArmorEnchantments(Level level, ItemStack armor, EquipmentSlot slot) {
        ...
        if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, armor, Enchantments.UNBREAKING, 3);
        ...
        switch (slot) {
            case HEAD:
                if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, armor, Enchantments.AQUA_AFFINITY, 1);
                if (RANDOM.nextBoolean()) applyRandomLevelEnchant(level, armor, Enchantments.RESPIRATION, 3);
                break;
            case CHEST:
                break;
            case LEGS:
                ...
                break;
            case FEET:
                // Now, roll a 1% chance to apply Frost Walker (rare), otherwise apply Depth Strider.
                if (RANDOM.nextInt(20) == 0) {
                    applyRandomLevelEnchant(level, armor, Enchantments.FROST_WALKER, 2);
                } else
                     if (RANDOM.nextInt(10) == 0) applyRandomLevelEnchant(level, armor, Enchantments.DEPTH_STRIDER, 3);

                if (RANDOM.nextInt(20) == 0) applyRandomLevelEnchant(level, armor, Enchantments.FEATHER_FALLING, 4);
                break;
            default: break;
        }
    }

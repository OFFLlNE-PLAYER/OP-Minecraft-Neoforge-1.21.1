...
public class PROCGunbladeProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), Objects.requireNonNull(BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("opminecraft:gunblade_slash"))), SoundSource.MASTER, (float) 0.5, (float) Mth.nextDouble(RandomSource.create(), 0.9, 1.1));
			}
		}
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == RegistryIBBI.GUNBLADE.get()) {
			...
			OPMinecraft.queueServerWork(3, () -> {
				entity.hurt(new DamageSource(world.holderOrThrow(DamageTypes.PLAYER_EXPLOSION)), Mth.nextInt(RandomSource.create(), 4, 14));

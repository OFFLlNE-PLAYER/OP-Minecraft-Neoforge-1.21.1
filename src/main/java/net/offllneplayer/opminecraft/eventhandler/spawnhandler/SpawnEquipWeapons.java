...
public final class SpawnEquipWeapons {
    ...
    // Equip pistol when roll <= 4 for any living mob, and return the weapon stack for main-hand.
    public static ItemStack setupPistol(LivingEntity pistolUser, int roll) {
        ItemStack weapon;
        if (roll < 4) {
            ...
            Registry<Item> itemRegistry = pistolUser.level().registryAccess().registryOrThrow(Registries.ITEM);
            ...
            for (Holder<Item> holder : pistolEntries) {
                seen++;
                if (pistolUser.getRandom().nextInt(seen) == 0) {
        ...
        if (!weapon.isEmpty()) {
            ...
            pistolUser.setItemInHand(InteractionHand.OFF_HAND, ammo);

            // Add pistol usage goal if entity is a Mob
            if (pistolUser instanceof Mob mob) {

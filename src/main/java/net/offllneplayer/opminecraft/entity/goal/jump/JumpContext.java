...
public final class JumpContext {
 ...
 public enum State { GROUNDED, AIRBORNE }
   ...
   // Capability flags
   public boolean hasBalloon = false; // has balloon check
   ...
   public boolean wasOnGround = true;
	...
	public Vec3 failLandingPos = Vec3.ZERO;
	public Vec3 failLandingPosOld = Vec3.ZERO;
	 ...
	 */
	public void recordAvoidLanding(Vec3 from) {
		if (from == null || from.equals(Vec3.ZERO)) return;
		if (this.failLandingPos != null && !this.failLandingPos.equals(Vec3.ZERO)) {
			this.failLandingPosOld = this.failLandingPos;
		}
		this.failLandingPos = from;
	}
	...
	public static void setupContext(JumpContext ctx) {
		...
		ctx.faceDir = ctx.mob.getDirection();

		ctx.jumpFromPos = ctx.mob.position();
		ctx.targetExactPos = ctx.mob.getTarget().position();

		ctx.hasBalloon = isBalloonAttacker(ctx.mob);
		ctx.hasRangedWep = isRangedAttacker(ctx.mob);

		ctx.maxUp = ctx.hasBalloon ? 3 : 1;
		ctx.maxGap = ctx.hasBalloon ? 5 : 2;
		ctx.maxDown = ctx.hasBalloon ? 14 : 6;
		ctx.COLLISION_HEIGHT = ctx.hasBalloon ? 3 : 2;

		// Wither Skeleton: taller collision height
		if (ctx.mob instanceof WitherSkeleton) {
			ctx.COLLISION_HEIGHT = ctx.hasBalloon ? 4 : 3;
		}

		// Compute hold gate for this tick
		...
		ctx.desiredYGain = ctx.hasRangedWep ? ((mobY >= targetY + 4) ? 0 : 4) : (mobY < targetY ? 1 : 0);
	}
	...
	public static boolean rangedHoldGate(JumpContext ctx, Direction grid) {
		...
		// Ranged distance band (applies to both balloon and non-balloon): hold position up to 16 blocks away from target
		double dist2 = JumpUtils.horizontalDistSqr(ctx.mob.position(), target.position());
		if (dist2 < JumpContext.RANGED_MAX_HOLD_CHECK_DIST
				&& JumpUtils.isNoPathHere(ctx.mob.level(), ctx.mob.blockPosition().relative(grid, 1), ctx.COLLISION_HEIGHT)
				|| JumpUtils.isGapOpen(ctx.mob.level(), ctx.mob.blockPosition().relative(grid, 1))) return true;
		return false;
	}
}
